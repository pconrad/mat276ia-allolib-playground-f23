...
const float xScale = 90.0;
const float yScale = 90.0;
const float zScale = 90.0;
...
public:
  ...
  void init() override
  {
    ...
    createInternalTriggerParameter("x", 0.0, -90.0, 90.0);
  ...
  void onProcess(Graphics &g) override
  {
    ...
    float z = getInternalParameterValue("z");
    ...
    //g.scale(1.0 / xScale, 1.0 / yScale, 1.0/zScale);
    //x= 90; y = 0; amplitude = 1;
    float longt = 6.2831853*x/360;
    float lat = 3.1415926*y/180;

    x = 100*cos(lat)*cos(longt);
    y = 100*cos(lat)*sin(longt);
    z = 100*sin(lat);

    //std::cout << " nx=" << x << " ny=" << y << " nz=" << z << std::endl;

    // Move x according to frequency, y according to amplitude
    g.translate(x/xScale, y/yScale, z/zScale);
...
public:
 ...
 // this font is for rendering text labels on the axes
  ...
  void onDraw(Graphics &g) override
  {
    ...
    drawXAxis(g, -90, 90, 30.0, tickSize, labelOffset, fontSize);
    drawZAxis(g, -90, 90, 30.0, tickSize, labelOffset, fontSize);
    drawWorldMap(g);
  ...
  )
  {
    ...
    bProgress.vertex(0, minX, -120);
    bProgress.vertex(0, maxX, -120);
    ...
    globalTime.update();
    Progress.vertex(0, minX, -120);
    double currTime = std::min((double)globalTime.now(), 60.0);
    Progress.vertex(0, minX + ((double)currTime * (maxX-minX))/60.0, -120);
                 ...
                 float fontSize)
  {
    ...
    // Create x and y axes
    xAxis.vertex(minX, 0, 0);
    xAxis.vertex(maxX, 0, 0);
                 ...
                 float fontSize)
  {
    ...
    // Create x and y axes
    zAxis.vertex(0, 0, minZ);
    zAxis.vertex(0, 0, maxZ);

    // Create the X tick marks and labels

    for (float x = 0; x < maxZ; x += tickIncrement)
    {
      zAxis.vertex(0, 0, x);
      zAxis.vertex(0, tickSize, x);
      zAxis.vertex(0, -tickSize, x);
      zAxis.vertex(0, 0, x);
      drawLabelX(g, x, labelOffset, fontSize);
    }

    for (float x = 0; x >= minZ; x -= tickIncrement)
    {
      zAxis.vertex(0, 0, x);
      zAxis.vertex(0, tickSize, x);
      zAxis.vertex(0, -tickSize, x);
                 ...
                 float fontSize)
  {
    ...
    yAxis.vertex(0, minY, 0);
    yAxis.vertex(0, maxY, 0);
...
void readPoints()
{
  ...
  while (std::getline(infile, line))
  {
    ...
    y = 100*cos(lat)*sin(longt);
    float z = 100*sin(lat);
    //std::cout << std::stof(parts[0]) << ' ' << std::stof(parts[1]) << ' '<< x <<' '<<y << ' ' << z <<'\n';
